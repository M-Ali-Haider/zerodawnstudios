...
const GlassHeader = ({ top = false }) => {
  return (
    <div
      className={`${
        top && "xs:hidden"
      } flex bg-[#6D6D6D33] backdrop-blur-3xl max-w-[448px] w-full justify-between items-center py-2 xs:py-4 px-3 xs:px-9 rounded-full`}
    >
      <div className="flex items-center gap-[10px] xs:gap-4">
        <LogoSVG className="w-[34px] h-[34px] xl:w-14 xl:h-14" fill="white" />
